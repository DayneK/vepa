...
> **Audit Hash:** `AGENTS_SYNC_v9.1.9_2026-09-20`
>
...
| **Active version** | **VEPA4 v9.1.6** (legacy label `4.9.9`) — operational WebGPU worker pairwise bridge with device-loss fallback status and exact CPU continuation |
...
**Version alignment (new schema since 2026-08-10):** the product is **VEPA4**; versions use `major.minor.build` (npm-semver-native). `VERSION`, the top section of `CHANGELOG.md` (arrow token), `package.json#version`, and this file **must all read 9.1.6** (the v9.1.6 WebGPU hardening build). Since the 2026-08-10 restructure the root manifest IS the v4 manifest (the legacy v2 root `package.json` was archived with the legacy trees).
...
├── style.css
├── package.json            ← v4 manifest (9.1.6 — MUST match changelog top)
├── package-lock.json
...
| **HEAD** | `d056de6` — `fix(physics-ui): preserve particle connections and simplify telemetry` |
...
| **Working tree** | clean for tracked files; pre-existing untracked `bun.lock`, `dist/`, and `isolate/` preserved |
...
*Reviewed 2026-09-17 | Workspace state: v9.1.6 WebGPU worker bridge and audit clarification prepared on top of `a9f3103` on `main`; changes remain uncommitted and untagged. Deterministic SharedArrayBuffer worker execution, completion-driven main-thread orchestration, and exhaustive full-stress benchmark SPA shipped. O·P·Q at build 2 of 3 — Set Q (cosmology, v8.17.0) next. Changelog snapshots `CHANGELOG.md.bak-20260819*` + `v4-worktree/` untracked/untouched. Verify SSOT parity before any code change.*
