...
> **Audit Hash:** `AGENTS_SYNC_v9.1.11_2026-09-20`
>
...
| **Active version** | **VEPA4 v9.1.11** (legacy label `4.9.14`) — active relationship-space exploration, mechanism templates, deterministic experiment batches, and outcome-regime clustering |
...
| **Branches** | `master` (stable) · `fix/particle-connection-semantics` (**current**) · `new` · `feature/slider-controls` · `feature/multiplayer-investigation` · `feature/nuclear-rewrite` (remote) |
...
**Version alignment (new schema since 2026-08-10):** the product is **VEPA4**; versions use `major.minor.build` (npm-semver-native). `VERSION`, the top section of `CHANGELOG.md` (arrow token), `package.json#version`, and this file **must all read 9.1.11** (the v9.1.6 WebGPU hardening build). Since the 2026-08-10 restructure the root manifest IS the v4 manifest (the legacy v2 root `package.json` was archived with the legacy trees).
...
├── style.css
├── package.json            ← v4 manifest (9.1.11 — MUST match changelog top)
├── package-lock.json
...
| Property | Value |
|----------|-------|
| **Current Branch** | `fix/particle-connection-semantics` |
| **HEAD** | `7ee9efc` — `feat(physics): formalize particle interaction space` |
...
| **Working tree** | tracked generated `dist/` changes plus untracked generated `dist/src/`, `public/src/`, `src/physics/relationshipCompatibility.js`, and `src/physics/relationshipState.js` preserved; no destructive cleanup performed |
...
| **Version drift** | `VERSION`/changelog arrow token/`package.json#version` must all read 9.1.6 (VEPA4 `major.minor.build`); keep them matched per §10.4 |
...
`VERSION`, the changelog top section's arrow token, and `package.json#version` MUST all equal the newest version (`9.1.6`). From here on, drift is a release
blocker.
...
*Reviewed 2026-09-20 | Workspace state: v9.1.11 relationship-space exploration is prepared on `fix/particle-connection-semantics`; changes remain uncommitted and untagged. The deterministic laboratory layer generates bounded relationship configurations, preserves named mechanism templates, and clusters measured outcome regimes without replacing solver semantics. Generated build artifacts remain untracked/preserved. Verify SSOT parity before any code change.*
