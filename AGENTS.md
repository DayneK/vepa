...
> **Audit Hash:** `AGENTS_SYNC_v9.1.10_2026-09-20`
>
...
| **Active version** | **VEPA4 v9.1.10** (legacy label `4.9.13`) — multidimensional relationship compatibility and first-class relationship-state utilities layered onto the particle connection solver |
...
**Version alignment (new schema since 2026-08-10):** the product is **VEPA4**; versions use `major.minor.build` (npm-semver-native). `VERSION`, the top section of `CHANGELOG.md` (arrow token), `package.json#version`, and this file **must all read 9.1.10** (the v9.1.6 WebGPU hardening build). Since the 2026-08-10 restructure the root manifest IS the v4 manifest (the legacy v2 root `package.json` was archived with the legacy trees).
...
├── style.css
├── package.json            ← v4 manifest (9.1.10 — MUST match changelog top)
├── package-lock.json
...
*Reviewed 2026-09-20 | Workspace state: v9.1.10 relationship compatibility work is prepared on `fix/particle-connection-semantics`; changes remain uncommitted and untagged. The solver now gates relationship formation using multidimensional compatibility while preserving ACCR edge seams, and generated specifications pass parity checks. Generated build artifacts remain untracked/preserved. Verify SSOT parity before any code change.*
