...
> **Working Branch:** `main`
> **Audit Hash:** `AGENTS_SYNC_v9.1.21_2026-09-21`
>
...
| **Active version** | **VEPA4 v9.1.21** (legacy label `4.9.24`) — systems atlas for organization, reproduction, lineage, culture, civilization proxies, and relationship-laboratory analysis |
...
**Version alignment (new schema since 2026-08-10):** the product is **VEPA4**; versions use `major.minor.build` (npm-semver-native). `VERSION`, the top section of `CHANGELOG.md` (arrow token), `package.json#version`, and this file **must all read 9.1.21** (the 9.1.16 systems-atlas assessment build). Since the 2026-08-10 restructure the root manifest IS the v4 manifest (the legacy v2 root `package.json` was archived with the legacy trees).
...
├── style.css
├── package.json            ← v4 manifest (9.1.21 — MUST match changelog top)
├── package-lock.json
...
|----------|-------|
| **Current Branch** | `main` |
| **HEAD** | `a0eb55e` — `Merge pull request #10 from DayneK/docs/roadmap-catalog-917` (working tree contains the uncommitted third-stagger implementation) |
| **Working tree** | third integrated stagger of the shared A-D variant substrate; preserved generated `dist/` artifacts remain untracked |
...
`VERSION`, the changelog top section's arrow token, and `package.json#version` MUST all equal the newest version (`9.1.21`). From here on, drift is a release
blocker.
