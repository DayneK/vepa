...
> **Working Branch:** `main`
> **Audit Hash:** `AGENTS_SYNC_v9.1.20_2026-09-21`
>
...
| **Active version** | **VEPA4 v9.1.20** (legacy label `4.9.23`) — systems atlas for organization, reproduction, lineage, culture, civilization proxies, and relationship-laboratory analysis |
...
**Version alignment (new schema since 2026-08-10):** the product is **VEPA4**; versions use `major.minor.build` (npm-semver-native). `VERSION`, the top section of `CHANGELOG.md` (arrow token), `package.json#version`, and this file **must all read 9.1.20** (the 9.1.16 systems-atlas assessment build). Since the 2026-08-10 restructure the root manifest IS the v4 manifest (the legacy v2 root `package.json` was archived with the legacy trees).
...
├── style.css
├── package.json            ← v4 manifest (9.1.20 — MUST match changelog top)
├── package-lock.json
...
`VERSION`, the changelog top section's arrow token, and `package.json#version` MUST all equal the newest version (`9.1.20`). From here on, drift is a release
blocker.
